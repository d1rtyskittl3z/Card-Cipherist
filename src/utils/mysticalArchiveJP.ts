/**
 * Mystical Archive JP Utilities
 *
 * Helper functions for managing the Japanese Mystical Archive frame stretching.
 * Similar to neoBasics.ts but handles both title bar height and type bar width.
 */
...
export const MYSTICAL_ARCHIVE_JP_FRAME_PREFIX = '/img/frames/mysticalArchive/jp/';

// Title bar height constants (in pixels, based on original 270px default)
export const MYSTICAL_ARCHIVE_JP_MIN_TITLE_HEIGHT = 100;
export const MYSTICAL_ARCHIVE_JP_MAX_TITLE_HEIGHT = 1000;
export const MYSTICAL_ARCHIVE_JP_DEFAULT_TITLE_HEIGHT = 270;

// Type bar width constants (in pixels, based on original 430px default)
export const MYSTICAL_ARCHIVE_JP_MIN_TYPE_WIDTH = 150;
export const MYSTICAL_ARCHIVE_JP_MAX_TYPE_WIDTH = 1000;
export const MYSTICAL_ARCHIVE_JP_DEFAULT_TYPE_WIDTH = 430;

// Base values for text configuration (normalized)
export const MYSTICAL_ARCHIVE_JP_BASE_TITLE_HEIGHT = 270 / 2100;
export const MYSTICAL_ARCHIVE_JP_BASE_TYPE_WIDTH = 430 / 1500;

const PATH_SPLIT_REGEX = /(?=[clmz])/gi;

export type MysticalArchiveJPChange = [number, number];

export const clampMysticalArchiveTitleHeight = (height: number): number => {
  if (Number.isNaN(height)) {
    return MYSTICAL_ARCHIVE_JP_DEFAULT_TITLE_HEIGHT;
  }
  return Math.min(
    MYSTICAL_ARCHIVE_JP_MAX_TITLE_HEIGHT,
    Math.max(MYSTICAL_ARCHIVE_JP_MIN_TITLE_HEIGHT, height),
  );
};

export const clampMysticalArchiveTypeWidth = (width: number): number => {
  if (Number.isNaN(width)) {
    return MYSTICAL_ARCHIVE_JP_DEFAULT_TYPE_WIDTH;
  }
  return Math.min(
    MYSTICAL_ARCHIVE_JP_MAX_TYPE_WIDTH,
    Math.max(MYSTICAL_ARCHIVE_JP_MIN_TYPE_WIDTH, width),
  );
};

/**
 * Compute the change value for the title bar (adjustable) stretch entry.
 * The change is applied vertically (y-axis).
 */
export const computeMysticalArchiveTitleChange = (height: number): MysticalArchiveJPChange => [
  0,
  (height - MYSTICAL_ARCHIVE_JP_DEFAULT_TITLE_HEIGHT) / 2100,
];
 ...
 */
export const computeMysticalArchiveTypeChange = (width: number): MysticalArchiveJPChange => [
  (width - MYSTICAL_ARCHIVE_JP_DEFAULT_TYPE_WIDTH) / 1260,
  0,
];
...
const transformSvgWithStretch = (
  ...
  card: Card,
  _colorOverrides?: Record<string, FrameColorOverride>,
): string => {
  ...
  stretch.forEach((stretchEntry) => {
    ...
    // Note: Mystical Archive JP doesn't use color overrides like NeoBasics,
...
export const generateMysticalArchiveStretchedImage = async (options: {
  src: string;
  card: Card;
  stretch: StretchConfig[];
